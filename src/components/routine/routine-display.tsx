...
const RoutineDisplay = forwardRef(({ scheduleData, timeSlots, classes, subjects, teachers, teacherSubjects, onScheduleChange, isPrintView = false }: RoutineDisplayProps, ref) => {
  ...
  const renderCellContent = (day: string, className: string, timeSlot: string) => {
    ...
    if (entries.length === 0) {
        return (
            ...
            >
                {!isPrintView && <span className="text-muted-foreground text-xs hover:text-primary opacity-50 hover:opacity-100">+</span>}
            </div>
        )
    }
    ...
    return (
        <div className="h-full min-h-[60px] flex flex-col items-center justify-center p-1 space-y-1">
            {uniqueEntries.map((entry, index) => (
                <div
                    key={index}
                    className={`w-full text-xs text-center p-1 bg-background rounded ${!isPrintView ? 'cursor-pointer hover:bg-accent hover:shadow-md' : ''}`}
